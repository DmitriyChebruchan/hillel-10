...
from .forms import PhoneNumberForm, VarificationForm

from .models import VerificationPair, PhoneNumber
...
def main(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            test_code = randint(99999, 100000)
            ...
            verification_pair = VerificationPair.objects.create(
                phone_number=the_number, code=test_code
            )
            send_sms(phone_number, test_code)
...
def verification(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            print(code_entered, code_sent)
            if code_entered == code_sent:
                verification_pair.delete()
                return render(request, 'verification_completed.html')
            return render(
                request,
